...
while 'MOLMS-FINANCE-SIDEBAR-V9B' in s:
    ...
    if runtime < 0:
        ...
        if end < 0:
            raise RuntimeError('unterminated V9B style block')
        end += len('</style>')
    else:
        end=s.find('</script>', runtime)
        if end < 0:
            raise RuntimeError('unterminated V9B runtime block')
        end += len('</script>')
    s=s[:start]+s[end:]
...
while 'MOLMS-SIDEBAR-MOTIVATION-V11B' in s:
    ...
    if start < 0 or end < 0:
        raise RuntimeError('unterminated V11B style block')
    end += len('</style>')
    s=s[:start]+s[end:]

# 4) Clean only known document-tail escaped newline artifacts.
s=s.replace('</script>\\n<style>', '</script>\n<style>')
s=s.replace('</style>\\n<script>', '</style>\n<script>')
s=s.replace('</script>\\n</body>', '</script>\n</body>')
s=s.replace('\\n</body>', '\n</body>')
