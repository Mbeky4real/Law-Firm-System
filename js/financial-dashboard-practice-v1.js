/* MOLMS Financial Dashboard Practice Classification V2
 * Practice Performance reads the same authoritative financial tables directly.
 * This avoids relying on legacy in-memory arrays that omit manual revenue rows.
 */
(function financialDashboardPracticeV2(){
  'use strict';
  ...
  async function loadRows(){
    ...
    try{
      ...
      const invoices=(inv.data||[]).filter(i=>!['draft','void','cancelled','superseded'].includes(String(i.status||'').toLowerCase())).map(i=>({
        kind:'invoice',currency:i.currency||'TZS',revenue:num(i.total_due),practice:classify(i),client:i.client_name||'Unknown client',source:i.invoice_number||'—'
      }));
      const manual=(man.data||[]).filter(t=>String(t.status||t.approval_status||(t.is_approved?'approved':'')).toLowerCase()==='approved').map(t=>({
        kind:'manual',currency:t.currency||'TZS',revenue:t.client_receivable&&num(t.agreed_amount)>0?num(t.agreed_amount):num(t.amount),practice:classify(t),client:t.receivable_client_name||t.counterparty||'Other revenue',source:t.reference||'Manual revenue'
      }));
      sourceRows=[...invoices,...manual];
      loaded=true;
    }catch(e){
      console.warn('[MOLMS practice classification V2]',e);
    }finally{
      loading=false;
    }
  }
  ...
  function render(){
    const host=q('fdRevByPractice');if(!host||!loaded)return;
    const currencies=['TZS',...new Set(sourceRows.map(r=>r.currency).filter(c=>c!=='TZS'))];
    const key=JSON.stringify([bounds(),sourceRows]);
    if(key===lastKey&&/Litigation/.test(host.textContent||''))return;
    lastKey=key;
    host.innerHTML=currencies.map((c,i)=>section(c,i?`${c} PRACTICE REVENUE`:'' )).join('');
  }

  async function refresh(){await loadRows();render();setTimeout(render,80);setTimeout(render,300)}

  window.addEventListener('load',()=>{setTimeout(refresh,700);setTimeout(refresh,1600)});
  setInterval(async()=>{await loadRows();render()},1200);
  window.MOLMSPracticeV2={classify,render,refresh,audit:()=>({period:bounds(),loaded,rows:sourceRows})};
})();